"""Onboarding curriculum: 3 continuous stages (A/B → C/D → E).

Stage 1 (A+B, iters ~1-40): expand teacher from tick 0. A is --bc-only;
B is PPO+SIL with λ_bc 1.0 → 0.05. No rifle rush schema.
Stage 2 (S+C+D, ~40-200): MAB over scripted bots (no cliff mix, no wipes).
Stage 3 (E, 200+): PFSP-RL league with frozen hist snapshots + 20% hard/medium.

Teacher is always mode=expand. Adam is never reset on promote (only on
true collapse in auto_train). Elite / teacher_wins accumulate.
"""
...
PHASES = ("A", "B", "S", "C", "D", "E", "done")
PHASE_BOT = {
    "A": "beginner", "B": "beginner", "S": "beginner",
    "C": "easy", "D": "medium", "E": "hard",
}
PHASE_NEXT = {
    "A": "B", "B": "S", "S": "C", "C": "D", "D": "E", "E": "done",
}
EXPAND_PHASES = ("C", "D", "E")
# Legacy name: all onboard phases now use the expand teacher.
RUSH_PHASES = ()
...
DEFAULTS = {
    ...
    "c_bc_lambda_start": 0.05,
    "c_bc_lambda_end": 0.05,
    "c_sft_iters": 15,
    "c_sft_done": False,
...
_STRIP = {
    ...
    "--bc-macro-ticks", "--bc-max-steps", "--bc-lambda-end", "--bc-lambda-start",
    ...
    "--mab", "--mab-tau", "--mab-floor", "--pfsp-hist-cap",
}
...
def save_curriculum(path: str | Path, cfg: dict) -> None:
    ...
    payload = {
        ...
        "s_promote_wr20": float(
            cfg.get("s_promote_wr20", DEFAULTS["s_promote_wr20"])),
...
def promote_threshold(cfg: dict, phase: str) -> float:
    ...
    if phase == "S":
        return float(cfg.get("s_promote_wr20", DEFAULTS["s_promote_wr20"]))
    if phase == "C":
...
def phase_flags(phase: str, cfg: dict) -> list[str]:
    ...
    if phase == "S":
        return _s_phase_flags(cfg)
    if phase in EXPAND_PHASES:
        return _expand_phase_flags(phase, cfg)
    return []


def _bc_start_at_floor(cfg: dict) -> int:
    """BC start_iter so λ is already at b_bc_lambda_end (no restart to ~1.0).

    Prefer explicit b_bc_start_iter (B origin). Else pin origin so the first
    iter of the current phase is past warmup.
    """
    bc_start = int(cfg.get("b_bc_start_iter") or 0)
    if bc_start > 0:
        return bc_start
    warmup = int(cfg.get("b_bc_warmup") or DEFAULTS["b_bc_warmup"])
    started = int(cfg.get("phase_started_iter") or 0) or 1
    return max(1, started + 1 - warmup)


def _s_phase_flags(cfg: dict) -> list[str]:
    """S: MAB ladder start (beginner/easy), expand BC at B λ floor."""
    cfg = _cfg(cfg)
    bc_start = _bc_start_at_floor(cfg)
    lam = float(cfg.get("b_bc_lambda_end", DEFAULTS["b_bc_lambda_end"]))
    return [
        "--bot-type", "beginner",
        "--pfsp", "--mab",
        ...
        "--bc-rush", str(int(cfg.get("a_rush") or DEFAULTS["a_rush"])),
        "--bc-warmup", str(int(cfg.get("b_bc_warmup") or DEFAULTS["b_bc_warmup"])),
        "--bc-lambda-end", "{:.2f}".format(lam),
        "--bc-lambda-start", "{:.2f}".format(lam),
        "--bc-start-iter", str(bc_start),
        "--bc-macro-ticks", str(int(cfg.get("a_macro_ticks") or DEFAULTS["a_macro_ticks"])),
        "--bc-max-steps", str(int(cfg.get("a_max_steps") or DEFAULTS["a_max_steps"])),
        "--bc-win-cap", str(int(cfg.get("bc_win_cap") or DEFAULTS["bc_win_cap"])),
...
def _expand_phase_flags(phase: str, cfg: dict) -> list[str]:
    """C/D: MAB scripted ladder. E: PFSP-RL league + 20% hard/medium anchors.

    λ_bc stays at the residual floor (0.05). No mix cliffs, no C-SFT.
    """
    bot = PHASE_BOT[phase]
    bc_start = _bc_start_at_floor(cfg)
    lam = float(cfg.get("b_bc_lambda_end", DEFAULTS["b_bc_lambda_end"]))
    lr = "1.0e-4" if phase == "C" else "2.0e-5"
    if phase == "C":
        pool, mab, pfsp_rl, anchor_p = "beginner,easy,medium", True, False, "0.00"
    elif phase == "D":
        pool, mab, pfsp_rl, anchor_p = "easy,medium,hard", True, False, "0.00"
    else:
        pool, mab, pfsp_rl, anchor_p = "rl,medium,hard", False, True, "0.20"
    flags = [
        "--bot-type", bot,
        "--pfsp",
        "--pfsp-pool", pool,
        "--pfsp-anchor-prob", anchor_p,
        "--bc",
        "--bc-teacher-bot", bot,
        "--bc-teacher-mode", "expand",
        "--bc-games", str(int(cfg.get("b_bc_games") or DEFAULTS["b_bc_games"])),
    ...
    if mab:
        flags.extend(["--mab", "--mab-tau", "0.25", "--mab-floor", "0.05"])
    if pfsp_rl:
        flags.extend(["--pfsp-rl", "--pfsp-hist-cap", "15"])
    return flags
...
ows. Ambiguous history: if
    ...
    for _ in range(len(PHASES) + 1):
        ...
        if not nxt:
            break
        prev = cfg.get("phase")
        prev_started = int(cfg.get("phase_started_iter") or 0)
        cfg["phase"] = nxt
        ...
        if nxt == "B":
            ...
            cfg["a_launched"] = True
        if nxt == "S" and prev == "B":
            # Pin B BC origin so S/C keep λ at floor (do not invent S on
...
def should_promote(cfg: dict, rows: list[dict], last_iter: int,
    ...
    if phase == "A":
        ...
        if wr20(results) >= thr:
            return "B"
        return None
    if phase not in ("B", "S", "C", "D", "E"):
        return None
    bot = phase_bot(phase)
    results = outcomes_from_rows(rows, bot, onboard_phase=phase)
    thr = promote_threshold(cfg, phase)
    if phase == "S":
        need = int(cfg.get("s_streak", DEFAULTS["s_streak"]))
        min_it = int(cfg.get("s_min_iters", DEFAULTS["s_min_iters"]))
    else:
...
def rewind_onboard(ckpt_dir: str | Path, keep_iter: int,
    ...
    if keep_iter <= sft or dest == "A":
        ...
        if cfg.get("phase") not in ("B", "S", "C", "D", "E"):
            raise ValueError(
                f"rewind > sft_iters solo en fase B/S/C/D/E "
