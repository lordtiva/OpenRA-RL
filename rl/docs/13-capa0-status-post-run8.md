...
>
> **Corte 1161 (Run 37 higiene completada → Fast 2-proc + Defensa total en casa):** 20 iters (1142–1161), wr ~39%, lose ~48%, incomplete ~13%. H/KL finitos, AMP estable (~70 s/update). Confirmado el techo empírico: `harvest_edge` −800 a −2200 (1 sola harv, cash=0 persistente) y `defense_loss` −2.9 por gotero defensivo de 6. Archivo: `rl/ckpts/Run 37 (a_short place-role-hygiene 1142-1161)/`. Resume **1141**. **Run 38 (A+B):** (1) **Fast 2-proc (Mejora A):** `MAX_SUPPORT_PROCS=2`, auto-proc encola la 2da refinería con el saldo remanente ($2.200 de $5.000 iniciales) tras el cuartel. Duplica cosechadoras nativas (2 harvs) y muelles de descarga. Cola de vehículos (`TRAIN harv`) opera en paralelo a cola de edificios. (2) **Defensa unificada (Mejora B):** `RAID_HOME_ORDERS = 24`, todas las unidades de combate ociosas en casa (`idles_home`) responden en bloque ante raids, eliminando el goteo de 6. Smoke `--iters 1161` desde 1141. Bar: `harvest_edge > 0`, `defense_loss → 0.0`.
